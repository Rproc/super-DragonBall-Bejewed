...
void Jogar()
{
	...
	SDL_Surface *pause=TTF_RenderText_Solid(font_Menu,"Pause",Cor_Pause);
			...
			if( event.type == SDL_MOUSEBUTTONDOWN )
		    {
  	    	...
  	    	if( event.button.button == SDL_BUTTON_LEFT )
				...
				else if(event.button.x > PauseRect.x && event.button.x < PauseRect.x+PauseRect.w && event.button.y > PauseRect.y && event.button.y < PauseRect.y+PauseRect.h)
				{
					...
					SDL_BlitSurface(img,NULL,screen,&rect);
					BlitaPontos();
					SDL_BlitSurface(pause,NULL,screen,&PauseRect);
...
void BlitaPontos ()
{
	...
	SDL_Surface* textSurface = TTF_RenderText_Solid(font_Pontos, "Score:", foregroundColor);
		...
		while(quit==false)
		{
			...
			while( SDL_PollEvent( &event ) )
			{
				if( event.type == SDL_KEYDOWN )
				{
					...
					text = TTF_RenderText_Solid( font_Recordes, temp, textColor );
				}
				if( event.type == SDL_QUIT )
				{
					//Quit the program
			...
			SDL_BlitSurface(fundo,NULL,screen,NULL);
			text2 = TTF_RenderText_Solid( font_Recordes,"Novo Recorde!!", textColor2 );
...
void Pause()
{
	...
	SDL_Rect p_image={50,70};
	SDL_Rect rect={50,70,40,40};
	SDL_Event event;
	int running=true;
	//PAUSE=
	...
	while(running==true)
	{
		SDL_BlitSurface(PAUSE,NULL,screen,&p_image);
		SDL_BlitSurface(joia[0],NULL,screen,&rect);
		while(SDL_PollEvent(&event))
		{
			if( event.type==SDL_QUIT)
			{
