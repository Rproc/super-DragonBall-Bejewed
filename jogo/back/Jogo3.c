...
void CarregaImagem (PECA matriz[8][8],int i, int j,SDL_Surface **joia);
void BlitaPontos ();
void BlitaTempo ();
...
int main(int argc,char **argv)
{
	...
	const int FPS = 10;
	//Uint32 start;
	SDL_Surface *gokutempo=NULL;
	gokutempo=IMG_Load("images/goku40.png");
	SDL_Rect gokurect={0,1};
	int gokucont=0,gokuycont=1;
	
	while(movimentos > 0 && combina>0 && (gokurect.x+gokurect.w < screen->w))// && (gokurect.x+gokurect.w < screen->w) )
	{
		int t1 = SDL_GetTicks();
		...
		if(gokucont==2)
		{
			gokurect.x+=1;
			gokucont=0;
			gokurect.y+=gokuycont;	
		
			if(gokurect.y==0 || gokurect.y == 2)
				gokuycont*=-1;
		}
		...
		{
			TTF_Font* font = TTF_OpenFont("rec.ttf", 60);
			SDL_Rect textLocation = { 300, 380, 0, 0 };
			SDL_Color foregroundColor = { 255, 10, 50 };

			SDL_Surface* textSurface = TTF_RenderText_Solid(font, "O Tempo Acabou!", foregroundColor);
...
void Menu()
{
	...
	Mix_Chunk *Som_de_Troca = Mix_LoadWAV( "sound/low.wav" );
	SDL_Rect dest;
	SDL_Rect Dfundo;
	SDL_Event event;
	int running=true;
	...
	SDL_BlitSurface(fundo, NULL, screen, &Dfundo);

	temp=IMG_Load("images/goku2.bmp");
...
void quebra(PECA peca[8][8])
{
	...
	blitar(peca);
	Mix_Chunk *Som_de_quebra=Mix_LoadWAV( "sound/explosao.wav" );;
...
void BlitaTempo ()
{
	...
	SDL_Rect TimeLocation = { 200, 50, 0, 0 };
	SDL_Color TimeColor = { 0, 0, 0 };
	textSurface = TTF_RenderText_Solid(font, segundos, TimeColor);
	SDL_BlitSurface(textSurface,NULL,screen,&TimeLocation);
...
void Efeito_Recorde(R_ARRAY *recordes)
{
	...
	if(array.vetor[array.n-1].pontuacao<pontuacao)
	{
		TTF_Init();
